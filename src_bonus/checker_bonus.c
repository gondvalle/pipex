...
char	**commands(t_arguments_bonus args, int argn, char **argv, char **env)
{
	char	**commands;
	int		i;

	i = -1;
	commands = ft_calloc(argn - 3 - args.here_doc + 1, sizeof(char *));
	if (!commands)
		ft_error(MALLOC, NULL);
	while (++i < argn - 3 - args.here_doc)
	{
		commands[i] = command(env, argv[i + 2 + args.here_doc]);
		if (!commands[i])
