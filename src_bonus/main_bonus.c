...
void	child_last(t_arguments_bonus args, int **fds, char **env, int argn)
{
	int	last;
	int	i;

	last = argn - 3 - args.here_doc - 2;
	i = -1;
	while (++i < last)
	{
		close(fds[i][READ]);
	...
	if (dup2(args.out_fd, STDOUT_FILENO) == -1)
		ft_error(DUP, NULL);
	close(args.out_fd);
	if (execve(args.commands[last + 1], args.splits[last + 1], env) == -1)
		ft_error(COMMAND, args.argv[last + 3]);
}
